defmodule Mix.Elixango do
  ...
  defp parse_repo([], []) do
    ...
    |> Enum.flat_map(fn app ->
      ...
      Application.get_env(app, :elixango_repos, [])
    end)
    |> Enum.uniq()
    |> case do
      [] ->
        ...
        repositories managed by those applications in your config/config.exs:
            ...
            config #{inspect hd(apps)}, elixango_repos: [...]
        """
        []
      repos ->
        repos
  ...
  def create_system_conn(config) do
    create_system_conn(config, [])
  end
  def create_system_conn(config, opts) do
    opts = Keyword.merge(opts, [database: "_system", name: :"system_conn"])
    create_conn(config, opts)
  end
  def create_conn(config) do
    create_conn(config, [])
  end
  def create_conn(config, opts) do
    config = Keyword.merge(config, opts)
    case Arangox.start_link(config) do
      {:ok, conn} ->
        {:ok, conn}
  ...
  def check_availability(conn) do
    case Arangox.get(conn, "/_admin/server/availability") do
      {:ok, %Arangox.Response{status: 200}} ->
        {:ok, :available}
      {:error, e = %Arangox.Response{}} ->
        {:error, e}
      _ ->
        {:error, :unavailable}
    end
  ...
  def create_new_database(conn, options) do
    case Arangox.post(conn, "/_api/database", options) do
      {:ok,
        %Arangox.Response{
          status: 201,
        }} -> {:ok, :created}
      ...
      {:error,
        e = %Arangox.Error{}} -> {:error, e}
      _ ->
        {:error, :unknown}
    end
  end

  def create_collection(conn, options) do
    case Arangox.post(conn, "/_api/collection", options) do
      {:ok,
        %Arangox.Response{
          status: 201,
        }} -> {:ok, :created}
